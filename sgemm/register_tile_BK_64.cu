...
// - `OUT_M = BLOCK_M * TILE_REG = 64`
// - `OUT_N = BLOCK_N * TILE_REG = 64`
// - 一个 block 负责输出 `64x64` 的 C 子块
// - thread block 大小是 `32x32=1024` 线程
// - 每个线程负责 `2x2` 个输出元素（寄存器里累计）
...
__global__ void matrix_multiplication_kernel(const float* A, const float* B, float* C, int M, int K, int N) {
    ...
    __shared__ float sdata_A[BLOCK_K][OUT_M + 1]; // reduce bank conflict
